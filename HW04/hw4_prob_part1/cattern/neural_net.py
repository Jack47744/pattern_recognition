...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    b1_reshape = b1.reshape((len(b1), -1))
    b2_reshape = b2.reshape((len(b2), -1))
    out1 = np.maximum(W1.T.dot(X.T) + b1_reshape, 0)
    # print(f'out1 transpose: \n{out1.T}')
    # print(f'out1 transpose shape: {out1.T.shape}')
    out2 = W2.T.dot(out1) + b2_reshape
    scores = out2.T
    ...
    scores_exp_sum = scores_exp_sum.reshape(len(scores_exp_sum), -1)
    # print(f'scores_exp_sum:\n{scores_exp_sum}')
    scores_softmax = scores_exp / scores_exp_sum
    # print(f'scores_softmax:\n{scores_softmax}')
    scores_softmax_log = np.log(scores_softmax)
    # print(f'scores_softmax_log:\n{scores_softmax_log}')
    y_label = np.zeros((y.size, y.max()+1))
    y_label[np.arange(y.size), y] = 1
    # print(f'y:\n{y}')
    # print(f'y_label:\n{y_label}')
    tmp = scores_softmax_log * y_label
    # print(f'tmp:\n{tmp}')
    loss = -np.sum(tmp) / len(y)
    ...
    #############################################################################
    grads['b2'] = np.sum(scores_softmax - y_label, axis=0) / N
    grads['W2'] = out1.dot(scores_softmax - y_label) / N + reg*W2
    tmp = W2.dot((scores_softmax - y_label).T)
    A, B = tmp.shape
    for i in range(A):
      for j in range(B):
        if out1[i, j] == 0:
          tmp[i, j] = 0
    grads['b1'] = np.sum(tmp, axis=1) / N
    grads['W1'] = X.T.dot(tmp.T)/N + reg*W1
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in range(num_iters):
      ...
      #########################################################################
      random_idx = np.random.choice(len(y), batch_size)
